---
title: "Analysis 1: MELD 3.0 variables only"
...
my_vars = c("MELD_BILIRUBIN", "log_MELD_BILIRUBIN", 
            "MELD_SERUM_SODIUM", "log_MELD_SERUM_SODIUM", 
            "MELD_INR", "log_MELD_INR", 
            "MELD_SERUM_CREAT", "log_MELD_SERUM_CREAT",
            "MELD_ALBUMIN", "log_MELD_ALBUMIN", 
            "FEMALE", 
...
For all machine learning models, we will include the following covariates, capped as they are form MELD 3.0: 
...
my_train_mat = data.frame(
  ...
  model.matrix(Surv(SurvTime_by90d, Death_by90d) ~ .^2, # All 2-way interactions
               data = my_train_df %>% dplyr::select(all_of(my_vars)))
) %>% 
  # Exclude the garbage interactions
  dplyr::select(-c(X.Intercept., 
                   MELD_BILIRUBIN.log_MELD_BILIRUBIN, 
                   MELD_SERUM_SODIUM.log_MELD_SERUM_SODIUM, 
                   MELD_INR.log_MELD_INR, 
                   MELD_SERUM_CREAT.log_MELD_SERUM_CREAT, 
                   MELD_ALBUMIN.log_MELD_ALBUMIN))
...
my_test_mat = data.frame(
  my_test_df %>% 
    dplyr::select(SurvTime_by90d, Death_by90d), 
  model.matrix(Surv(SurvTime_by90d, Death_by90d) ~ .^2, # All 2-way interactions
               data = my_test_df %>% dplyr::select(all_of(my_vars)))
) %>% 
  # Exclude the garbage interactions
  dplyr::select(-c(X.Intercept., 
                   MELD_BILIRUBIN.log_MELD_BILIRUBIN, 
...
# 1 hour + 10 minutes to run (including refitting and predicting)
set.seed(1)
...
# Fit auto-tuners to training data
# ~1 hour + 20 minutes collectively (including benchmarking), with large 
